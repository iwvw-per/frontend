...
const productToForm = (p: Product): ProductFormState => ({
  ...
  size: p.props?.size != null ? String(p.props.size / GB) : "",
...
const ProductEditDialog = ({ open, editing, type, onClose, onSaved }: ProductEditDialogProps) => {
  ...
  const handleSave = () => {
    ...
    const service: UpsertProductService = {
      ...
      props: {
        size: form.size !== "" ? Math.round(parseFloat(form.size) * GB) : undefined,
  ...
  return (
    <DraggableDialog
      title={editing ? t("settings.editStorageProduct") : t("vas.addStoragePack")}
      showActions
      showCancel
      onAccept={handleSave}
      loading={saving}
      ...
      <DialogContent>
        <Box component="form" ref={formRef} sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
          ...
          <Grid2 container spacing={2} size={{ xs: 12 }}>
            ...
            <SettingForm title={t("vas.size")} lgWidth={6} noContainer>
              <FormControl fullWidth>
                <DenseFilledTextField
                  required
                  type="number"
          ...
          <SettingForm title={t("vas.durationDay")} lgWidth={12}>
            <FormControl fullWidth>
              <DenseFilledTextField
                type="number"
              ...
              />
              <NoMarginHelperText>{t("vas.durationGroupDes")}</NoMarginHelperText>
            </FormControl>
          </SettingForm>
...
const StorageProducts: React.FC<StorageProductsProps> = ({}) => {
  const { t } = useTranslation("dashboard");
  const dispatch = useDispatch<ThunkDispatch<any, any, AnyAction>>();
  const { enqueueSnackbar } = useSnackbar();
        ...
        <Table sx={{ width: "100%" }} size="small">
          <TableHead>
            <TableRow>
              ...
              <NoWrapCell>{t("vas.size")}</NoWrapCell>
          ...
          <TableBody>
            ...
            {products.map((p) => (
              <TableRow key={p.id}>
                ...
                <NoWrapCell>{p.props?.size != null ? `${p.props.size / GB}GB` : "-"}</NoWrapCell>
